...
def _instructions(req: VisionEvaluationRequest) -> str:
    return (
        ...
        "CANONICAL SOURCE: real-product-v1 (real photographs of the actual "
        "sold product, see the handle geometry reference crop) — NOT any "
        "AI-rendered reference. The canonical handles are FLAT CORNER TABS: "
        "outer silhouette flat, sitting almost FLUSH with the top rim "
        "(built into the wall's corner curve, NOT rising as a tall loop/arch "
        "above the rim), with a SHORT HORIZONTAL rectangular/slightly "
        "rounded cut-out (NOT a tall vertical oval), left and right handles "
        "consistent with each other within natural perspective. Compare "
        ...
        "visible). If a handle is a TALL vertical loop/arch rising above the "
        "rim, has an elongated VERTICAL oval cut-out, is rounded, puffy, "
        "arched, curved, hook-shaped, teardrop-holed, shortened, thickened, "
        "or merges into a rounded rim — that is hard fail "
        "handle_geometry_mismatch EVEN IF there are exactly two handles "
        "(this includes the OBSOLETE legacy AI geometry — tall loop handle "
        "with vertical oval cut-out — which does NOT match the real product "
        "and must never be treated as correct). List concrete deviations in "
...
def _handle_check_instructions() -> str:
    ...
    return (
        ...
        "the CANONICAL handle reference crop — a REAL PHOTOGRAPH of the "
        "actual sold product (real-product-v1), not an AI rendering.\n"
        "The canonical handle is a FLAT CORNER TAB: outer silhouette flat, "
        "almost FLUSH with the top rim, built into the wall's corner curve "
        "(NOT a tall loop/arch rising above the rim), with a SHORT "
        "HORIZONTAL rectangular/slightly rounded cut-out (NOT a tall "
        "vertical oval), left and right handles consistent within natural "
        "perspective.\n"
        "Judge ONLY the handle silhouettes against the reference: a tall "
        "vertical loop/arch handle, a vertical-oval cut-out, rounded, puffy, "
        "arched, curved, hook-shaped, teardrop-holed, shortened or thickened "
        "handles, or left/right inconsistency beyond natural perspective — "
        "all mean NO MATCH even with exactly two handles. This explicitly "
        "includes the OBSOLETE legacy AI geometry (tall loop handle, "
        "vertical oval cut-out) — treat it as a mismatch, never as correct. "
        "Doubt counts AGAINST the candidate. If a handle is occluded by a "
        "hand, judge the visible part and lower confidence. Respond ONLY "
