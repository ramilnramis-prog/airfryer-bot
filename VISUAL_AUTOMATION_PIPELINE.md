...
scene-specs/scene-NN.json  ←── Visual Bible (assets/visual-bible/<product>/)
        │
        ▼
image-producer ── api/media_pipeline (OpenAI Images API, gpt-image-1)
        │           3 кандидата/сцену, reference images, high input fidelity
...
| `openai_images_client.py` | OpenAIImagesProvider: generations + edits (несколько reference images, `input_fidelity=high`), бюджет-гейт, лимит кандидатов |
...
| `cli.py` | `plan` / `generate [--apply]` / `qa` / `sequence-qa` — структурированный JSON, коды выхода 0/1/2 |
...
- Бюджет-гейт: оценка ~$0.30/изображение, лимит на запуск (по умолчанию $5,
  у кампании cap $10); превышение = BudgetExceededError до любых вызовов.
...
```bash
# план и смета (без сети)
python -m api.media_pipeline.cli plan content/autopilot/coating-protect-2026-07

# генерация кандидатов сцены (реально и платно — только с --apply)
