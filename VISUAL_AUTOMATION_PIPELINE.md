...
форма ≠ эталон · ручек не ровно 2 · геометрия ручек ≠ канонический референс
(`handle_geometry_mismatch`: округлые/пухлые/изогнутые ручки, непараллельные
стороны, некапсульный вырез, асимметрия — count == 2 НЕ достаточен; в анимации
дрейф силуэта между кадрами = `handle_geometry_drift`) · изменён цвет/материал ·
другой аэрогриль ·
мужские руки · плохая анатомия рук · захват не за ручки · изменилось число еды
(только при подтверждённом подсчёте) · текст/watermark · невозможные
пересечения · CGI-вид · кадр неанимируем · кадр нарушает СОБСТВЕННЫЙ scene spec
(`current_scene_violation`) · невозможен естественный переход в следующую сцену
(`transition_impossible`). Плюс сценные условия из scene spec (например,
«форма НЕ должна быть в кадре» для сцен 1-2, 6).

**Семантика переходов**: следующая сцена описывает состояние ПОСЛЕ завершения
движения текущей. Она используется только чтобы проверить, что кадр можно
логично анимировать в это состояние (совпадают immutable-объекты, переход
физически возможен). То, что финальное состояние следующей сцены ещё не
наступило в текущем кадре, — ожидаемо и дефектом НЕ является (информационный
флаг `next_scene_state_not_yet_present`). Старый код `adjacent_scene_break`
удалён: он ложно требовал состояние следующей сцены уже в текущем кадре.

**Подсчёт еды**: основной evaluator возвращает `food_count_detail`
(visible/partially_occluded/uncertain/expected, confidence, evidence,
расположение каждого элемента, region формы). Картофель, гарнир, тени и части
одного элемента отдельными единицами не считаются; при перекрытии еды точное
число с высокой уверенностью запрещено. При confidence < 0.85, расхождении с
expected_count или uncertain > 0 запускается second-pass подсчёт на crop
области формы (только целевая еда). Расхождение проходов → статус
`food_count_uncertain`: победитель автоматически НЕ утверждается, неверное
число НЕ фиксируется.
