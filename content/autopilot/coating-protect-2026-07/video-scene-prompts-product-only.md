...
- **scene_goal**: "денежный" кадр — руки достают форму за плоские ручки, блюдо внутри, корзина чистая. **Стратегия — PRODUCT-PLACEMENT PLATE, revision C2 (angled/asymmetric framing)** (не "нарисуй финальный товар"): AI генерирует только plate (кухня/руки/корзина/композиция под вставку), финальная форма вставляется отдельным deterministic шагом ПОСЛЕ генерации — см. `product_lock_instruction` ниже (pipeline-only, НЕ отправляется модели) и `scene-05-product-only-apply-dry-run.json`. **C2 root cause fix:** C1 (frontal basket, level hand tabs) и его V2 local-transform salvage были отклонены — `real-product-v1/product_45deg` снят под 3/4 ракурсом с асимметричной высотой ручек (левая выше, правая ниже), а C1's AI plate рисовал фронтальный ракурс с ручками на одном уровне; ни один rigid transform (только uniform scale + малый rotation + translation, без warp) не может согласовать эти две геометрии одновременно. C2 просит AI сразу рисовать high 3/4 top-down ракурс с диагональной осью upper-left→lower-right и асимметричными руками, совместимый с `product_45deg` + `APPROVED_TRANSFORM_B` без per-plate transform.
- **image_prompt**: High 3/4 top-down camera angle looking down and slightly forward into the open air fryer basket, matching a diagonal product axis from the upper-left to the lower-right of the frame, not a frontal straight-on view. The woman's hands, in grey ribbed sweater sleeves, are positioned asymmetrically along that diagonal as if lifting an object out of the basket: the left hand is higher in the frame, near the upper-left corner of the basket where the future flat handle tab will be, and the right hand is lower in the frame, near the lower-right corner of the basket where the other future flat handle tab will be; thumbs angled toward those future flat handle tab positions. This leaves a clean, empty central placement area in the basket, aligned with the diagonal axis, for a square silicone liner to be inserted later. In the central placement area, include a loose cluster of exactly 3 roasted golden chicken thighs with potato wedges, positioned where the liner interior will be after compositing. Do not draw a completed silicone liner. Do not draw redesigned or fake handles. Do not draw any duplicate tray or basket insert. Do not draw a black insert or black liner inside the basket. Only very soft contact shadows/cues are allowed in the empty placement area. Around and below the placement area, the black air fryer basket is visibly clean and shiny.
