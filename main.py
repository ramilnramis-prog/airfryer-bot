...
async def prepare_telegram_post(request: Request, _=Depends(require_api_key)):
    # тело опционально: recipe_id можно не присылать (сервер возьмёт случайный рецепт)
    try:
    ...
    if not isinstance(payload, dict):
        payload = {}
    res = prepare_post(payload.get("recipe_id"), _base_url(request))
    if res is None:
